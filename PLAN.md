...
- Admin : `/admin` protégé par mot de passe (cookie signé HMAC). Dashboard avec bandeau de stats (leads/nouveaux cette semaine/en cours/gagnés/taux de conversion), recherche + filtre par statut, pipeline visuel (leads groupés par colonne de statut). Chaque lead a sa propre page de détail (`/admin/leads/[id]`, pas une carte dépliable) : statut/notes éditables, lien de preview du site, génération de liens de paiement Stripe, lien de suivi client toujours visible (pas seulement juste après une mise à jour), horodatage "dernière consultation par le client", partage d'images/logo (Cloudflare R2), composeur de mises à jour de projet, export CSV.
...
- Suivi de projet : 4 étapes client-facing avec un texte propre à cette page (pas une réutilisation du texte marketing de l'accueil — voir la note de ton ci-dessous), panneau résumé (pack + date de début relative), bouton d'aperçu du site si un lien de preview est renseigné, panneau "où en est votre projet aujourd'hui" mettant en avant l'étape en cours, galerie d'images/maquettes partagées (si le studio en a ajouté), fil chronologique des mises à jour postées depuis l'admin (table D1 `project_updates`) — page publique sans authentification, le token fait office de secret. Chaque visite met à jour un horodatage "dernière consultation", visible côté admin.
- **Ton du contenu client-facing** : les étapes de `trackingPage` ont leur propre texte (pas `home.processSteps`) — le texte de l'accueil est écrit pour convaincre un prospect ("pas une fourchette qui explose en cours de route") et sonnait abrupt une fois adressé à un client déjà signé. Toujours garder cette distinction pour tout futur texte destiné à un client déjà engagé : ton informatif et rassurant, pas persuasif/contrastif.
- Partage de fichiers : bucket R2 `calyroc-project-files` (`env.PROJECT_FILES`), images/logos uniquement (PNG/JPEG/WebP/SVG, 8 Mo max), stockés sous `leads/{token}/{fichier}` — la route de service publique (`/api/files/[token]/[key]`) est volontairement non authentifiée mais reste sécurisée par le token, même principe que `/suivi/[token]` lui-même.
- **Quand facturer le solde** (question tranchée après recherche) : juste après que le client valide formellement le site sur le lien de preview, avant l'ouverture d'une fenêtre de retouches et avant la mise en ligne finale — pas avant (le client n'a pas encore vu le résultat), pas après (aucun levier restant, risque que chaque petite retouche demandée retarde indéfiniment le paiement). Le bouton de validation client sur `/suivi/[token]` (pose `leads.client_approved_at`) est le signal qui déclenche ce moment ; l'admin affiche un badge "Approuvé le X" à cet instant précis. Le paiement lui-même se fait ensuite depuis `/suivi/[token]/paiement`, qui liste l'historique des paiements et crée une session Stripe fraîche à chaque clic sur "Payer maintenant" (les sessions Stripe Checkout expirent au bout de 24h maximum, donc un lien généré une fois dans l'admin ne peut pas rester affiché indéfiniment sur cette page — voir `src/app/[locale]/suivi/[token]/actions.ts`).
...
- Remplacement de `next-view-transitions` (librairie tierce, antérieure au support natif de Next 16) par le système natif `experimental.viewTransition` de Next.js — cause racine de gels de navigation intermittents (clic sur lien, changement de langue, bouton retour) signalés par l'utilisateur, confirmée par plusieurs tickets ouverts sur le dépôt Next.js décrivant le même symptôme. Le CSS de transition (`::view-transition-old/new(root)` dans `globals.css`) n'a pas eu besoin de changer, il cible l'API navigateur standard.
- Correctif du sélecteur de langue (`SiteHeader.tsx`) : changer de langue redirigeait systématiquement vers l'accueil au lieu de rester sur la page équivalente traduite. Nouvelle fonction `localizePath()` (`src/i18n/routes.ts`) qui ne traduit que le premier segment d'URL s'il correspond à une route localisée connue, laissant le reste (slug d'article de blog, token de suivi) intact.
- Article de blog traduit en anglais (auparavant français uniquement, malgré l'interface traduite) ; les visiteurs dans une langue autre que fr/en sont désormais redirigés vers la version anglaise plutôt que de voir du texte français sous une interface traduite.

**Audit SEMrush (2026-07-17)** — suite à la demande de l'utilisateur de comparer avec le score Ahrefs 100/100 :
- Le score de santé SEMrush (83%, 253 erreurs, 111 avertissements) est trompeur : 252 des 253 erreurs proviennent d'un seul incident de timing — le crawl SEMrush est tombé pile pendant l'un des multiples déploiements Cloudflare Workers effectués ce jour-là, capturant une quinzaine de pages (routes traduites) en erreur 503 transitoire. Confirmé par revérification manuelle de toutes les URLs concernées (200 OK), y compris avec un user-agent `SemrushBot`. La dernière erreur ("format robots.txt invalide") est la ligne `Content-Signal` ajoutée automatiquement par Cloudflare AI Crawl Control — pas reconnue par le parseur (plus ancien) de SEMrush, rien à corriger côté code.
- Les avertissements (79 pages "faible ratio texte/HTML", 32 pages "peu de mots") sont très majoritairement des faux positifs structurels : le premier touche 79 pages sur 80 (tout site en CSS utilitaire + animations déclenche ce critère obsolète, qui n'est plus un facteur de classement Google) ; le second touche des types de page intrinsèquement courts par nature (liste du blog, formulaire de contact, mentions légales).
- Quota mensuel SEMrush (100 pages/mois, forfait gratuit) épuisé par ce premier crawl — impossible de relancer un crawl de vérification avant renouvellement ou passage à un forfait payant.
- HSTS relevé de 1 jour à 6 mois (`max-age=15552000`) après confirmation de la stabilité HTTPS/Cloudflare tout au long de la construction du site — voir commentaire dans `next.config.ts` pour le détail des risques (irréversibilité côté navigateur d'un `max-age` long, raison de ne pas encore ajouter `includeSubDomains`/`preload`).
- SEMrush Position Tracking (forfait gratuit) est limité à **une seule cible** (une combinaison pays/langue) par projet, contrairement à Ahrefs — passer à un forfait payant ("Guru") débloquerait jusqu'à 10 cibles, soit environ une par langue du site, mais reste une décision business (SEMrush est un outil d'analyse, pas ce qui fait le référencement lui-même) : non fait, en attente d'arbitrage.
- Recherche indépendante des outils SEO (pondération du référencement local, risques du contenu multilingue non révisé, signaux E-E-A-T) : Google Business Profile pèse à lui seul davantage que tout ce que mesurent SEMrush/Ahrefs combinés pour le pack local ; ajout d'une signature d'auteur visible sur les articles de blog (`blogPage.byLabel`, lien vers `/a-propos`) en réponse au signal E-E-A-T "auteur identifiable".

**Pages de service dédiées, 9 langues (2026-07-17)** — en réponse à la conclusion de l'audit ci-dessus (« une page /services générique liste tout — une page dédiée par service est un facteur de classement de premier plan ») :
- Chacun des 7 services (site vitrine, e-commerce, refonte, landing page, maintenance, SEO technique, identité visuelle) a désormais sa propre page approfondie sous `/services/[slug]` — contenu entièrement rédigé en anglais (langue prioritaire) puis localisé nativement dans les 9 langues, pas une traduction littérale (`src/content/services/*.ts`).
- Chaque page contient : hero, description, liste "inclus", étapes du process, FAQ (schéma `FAQPage`), encart croisé vers le pack tarifaire correspondant, grille de renvoi vers les autres services, CTA — pensé pour le scan humain autant que pour l'extraction par les moteurs IA (paragraphes courts, FAQ répondant dès la première phrase).
- URL localisée par slug (`serviceSlugs` dans `src/i18n/routes.ts`), même convention que le reste du site : dossier physique gardé en français, `rewrites()` dans `next.config.ts` fait correspondre chaque langue à ce dossier. `localizePath()` (sélecteur de langue) étend sa logique pour traduire aussi ce deuxième segment d'URL.
- Sitemap et grille `/services` mis à jour pour pointer vers les nouvelles pages (63 URLs au total : 7 services × 9 langues).

**Affinage post-lancement des pages de service (2026-07-17)** — suite au retour de Thomas après mise en ligne :
- Les cartes de la grille `/services` n'indiquaient pas visuellement qu'elles étaient cliquables — ajout d'un repère permanent "En savoir plus →" (`servicesPage.learnMoreLabel`) en bas de chaque carte, pas seulement au survol.
- Chaque page de service dédiée pointait vers sa propre FAQ mais jamais vers la FAQ générale du site — ajout d'un lien "Voir toutes les questions" sous chaque FAQ de service (réutilise `pricingPage.faqSeeMoreLabel`, déjà utilisé sur `/tarifs`).
- La page `/faq` elle-même paraissait maigre une fois les 7 pages de service enrichies : ajout de 5 questions générales qui font le pont entre les services ("quel service me correspond ?", combinaison de services, e-commerce vs vitrine, amélioration partielle d'un site existant, démarrage progressif) dans les 9 langues, plus un second bouton "Découvrir nos services" (`faqPage.servicesLinkLabel`) à côté du CTA de contact existant.
- Corrigé au passage : une question de la FAQ générale affirmait encore "disponible en 6 langues" (français, anglais, espagnol, italien, allemand, portugais) — obsolète depuis l'ajout du néerlandais/polonais/russe (voir §7 plus haut). Mise à jour vers 9 langues dans les 9 fichiers de dictionnaire.
- Retour de Thomas sur cette première passe : les FAQ de chaque page de service (4 questions par service, déjà écrites dans `src/content/services/`) restaient invisibles depuis `/faq` — seules les 12 questions générales du dictionnaire y apparaissaient. Avec 7 services, ça fait 28 questions supplémentaires à intégrer, trop pour une simple liste à plat. Refonte de `/faq` : nouveau composant client `FaqExplorer` qui regroupe tout par catégorie (Général + une par service, chacune avec un lien "En savoir plus" vers sa page), avec une barre de recherche qui filtre en direct sur l'ensemble des 40 questions (texte de la question et de la réponse) et des pastilles de catégorie pour restreindre l'affichage. Le contenu de chaque service reste dans `src/content/services/` (pas dupliqué dans le dictionnaire) — la page agrège, elle ne recopie pas. Le JSON-LD `FAQPage` couvre désormais les 40 questions pour correspondre à ce qui est visible.
- Chatbot Ask Calyroc (`src/i18n/chatContext.ts`) mis à jour pour connaître les 7 pages de service dédiées : nouvelle `servicesLinkTable` (un lien par service, par langue, construite depuis le barrel `src/content/services/`) injectée dans le `<context>` du system prompt, avec une règle explicite dans `<format>` — une question sur un service précis doit toujours lier vers SA page dédiée, pas seulement vers `/services` en général. Le lien vers `/faq` a aussi été ajouté à la table de liens générale, pour rediriger proprement une question hors du périmètre déjà couvert par le contexte.
- Documentation mise à jour en conséquence : `AGENTS.md` (arborescence, checklist chatbot), `docs/ARCHITECTURE.md` (nouvelle section "Pages de service dédiées", JSON-LD), `docs/CONTENT_GUIDE.md` (nouvelle section détaillant `src/content/services/`, procédure "ajouter un 8e service", table des composants dérivés mise à jour, politique de rédaction anglais-first pour tout nouveau contenu à partir de cette date).
