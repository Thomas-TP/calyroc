    ...
    <div className="mt-4 flex flex-col divide-y divide-paper/8">
      {items.map((item) => (
        <details key={item.question} className="group py-4">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-display text-sm font-medium text-paper transition-colors hover:text-bronze marker:content-none [&::-webkit-details-marker]:hidden">
            {item.question}
            <span
              aria-hidden
